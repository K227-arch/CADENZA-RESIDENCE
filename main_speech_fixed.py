...
# Add the speech client script
@app.get("/speech_client.js")
async def get_speech_client_js():
    with open("speech_client.js", "r") as f:
        return HTMLResponse(content=f.read(), media_type="application/javascript")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def send_text_for_speech_response(session, text):
        ...
        try:
            ...
            # Create a proper content message with correct Part syntax
            content = types.LiveClientContent(
                turns=[
                    types.Content(
                        parts=[types.Part(text=text)],  # Fixed: use text= parameter
                        role="user"
                    )
                ],
                turn_complete=True
    ...
    async def handle_gemini_response(session):
        ...
        try:
            async for response in session.receive():
                ...
                if hasattr(response, 'server_content') and response.server_content:
                    ...
                    if hasattr(server_content, 'model_turn') and server_content.model_turn:
                        ...
                        for i, part in enumerate(model_turn.parts):
                            print(f"📦 Processing part {i+1}: {type(part)}")
                            ...
                            if hasattr(part, 'inline_data') and part.inline_data:
                                if hasattr(part.inline_data, 'data') and part.inline_data.data:
                                    ...
                                    print("📦 Part has inline_data but no data")
                ...
                # Handle any direct audio data
                if hasattr(response, 'data') and response.data:
                    print(f"🔊 Direct audio data: {len(response.data)} bytes")
